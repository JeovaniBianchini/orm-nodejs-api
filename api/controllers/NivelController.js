const database = require('../models')  //importando a parte de modelos

class NivelCcontroller {
    static async pegaTodasOsNiveis(req, res){
        try{
            const todosOsNiveis = await database.Niveis.findAll()
            return res.status(200).json(todosOsNiveis)
        } catch (error) {
            return res.status(500).json(error.message)
        }
        
    }

    static async pegaUmNivel(req, res) {
        const { id } = req.params
        try{
            const umNivel = await database.Niveis.findOne( { 
                where: {
                    id: parseInt(id)
                }
            })
            return res.status(200).json(umNivel)
        }   catch (error) {
            return res.status(500).json(error.message)
        }
    }

    static async criarUmNivel(req, res){
        const novoNivelCriado = req.body
        try{
            const nivelCriado = await database.Niveis.create(novoNivelCriado)
            return res.status(201).json(nivelCriado)
        } catch (error) {
            return res.status(500).json(error.message)
        }
    }

    static async atualizaNivel(req, res){
        const { id } = req.params
        const novasInfos = req.body
        try{
            await database.Niveis.update(novasInfos, { where: { id: Number(id)}})
            const nivelAtualizado = await database.Niveis.findOne( { where: { id: Number(id)}})
            return res.status(200).json(nivelAtualizado)
        } catch(error){
            return res.status(500).json(error.message) 
        }
    }

    static async deleteUmNivel(req, res) {
        const { id } = req.params
        try{
            await database.Niveis.destroy( { 
                where: {
                    id: parseInt(id)
                }
            })
            return res.status(200).json(`Nivel com o id: ${id} foi deletado`)
        }   catch (error) {
            return res.status(500).json(error.message)
        }
    }
}

module.exports = NivelCcontroller